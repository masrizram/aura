...
function Initialize-AuraModules {
    <#
    .SYNOPSIS
        Loads AURA engine modules in deterministic dependency order.
    .DESCRIPTION
        Resolves engine root, validates module files, loads them in the correct
        dependency order, and verifies required commands exist after loading.
        Fails closed if any required module or command is missing.
    #>
    [CmdletBinding()]
    param()
    ...
    foreach ($moduleName in $moduleFiles) {
        $modulePath = Join-Path $ModulesDirResolved $moduleName
        if (Test-Path -LiteralPath $modulePath) {
            try {
                . $modulePath
                $loadedCount++
                Write-Verbose "[AURA] Loaded module: $moduleName"
            } catch {
                $failedModules += "$moduleName (error: $_)"
                Write-Host "[AURA] MODULE LOAD FAILURE: $moduleName -- $_" -ForegroundColor Red
            }
        } else {
            $failedModules += "$moduleName (file not found)"
            Write-Host "[AURA] MODULE MISSING: $moduleName" -ForegroundColor Yellow
        }
    }

    Write-Verbose "[AURA] Modules loaded: $loadedCount / $($moduleFiles.Count)"

    if ($failedModules.Count -gt 0) {
        $joined = $failedModules -join "; "
        Write-Host "[AURA] MODULE_DEPENDENCY_FAILURE: $($failedModules.Count) module(s) could not be loaded: $joined" -ForegroundColor Red
    }
    ...
    foreach ($cmdName in $requiredCommands) {
        if (-not (Get-Command $cmdName -ErrorAction SilentlyContinue)) {
            $missingCommands += $cmdName
        }
    }

    if ($missingCommands.Count -gt 0) {
        $joined = $missingCommands -join ", "
        Write-Host "[AURA] REQUIRED COMMAND VALIDATION FAILURE: $joined not available. Check that run-audit.ps1 defines these functions before module loading." -ForegroundColor Red
    }
...
# Module initialization is deferred to after all orchestrator functions
# are defined (see Initialize-AuraModules call near MAIN section).

function Bootstrap-EngineRoot {
    param(
        [string]$SourceRoot,
        [string]$ProjectPath
    )
    $engineRoot = Join-Path $ProjectPath ".aura"
    New-Item -ItemType Directory -Force -Path $engineRoot | Out-Null

    $sourceDirs = @("agents", "docs")
    foreach ($dir in $sourceDirs) {
        $targetDir = Join-Path $engineRoot $dir
    ...
    $configTarget = Join-Path $engineRoot "config.json"
    if (-not (Test-Path -LiteralPath $configTarget)) {
        $configSource = Join-Path $SourceRoot "config.json"
        if (Test-Path -LiteralPath $configSource) {
            Copy-Item -LiteralPath $configSource -Destination $configTarget -Force
        }
    }

    New-Item -ItemType Directory -Force -Path (Join-Path $engineRoot "state") | Out-Null
    New-Item -ItemType Directory -Force -Path (Join-Path $engineRoot "reports") | Out-Null
...
function Write-StateWithValidation {
    param(
        [string]$FindingsPath,
        [PSCustomObject]$ProposedFindingsData,
        [string]$ConvergencePath,
        [PSCustomObject]$ProposedConvergenceData,
        [string]$CyclePath,
        [PSCustomObject]$ProposedCycleData,
        [string]$ProjectPath,
        [int]$AuditedFileCount,
        [switch]$ForceValidation
    )
    ...
    $existingFindings = Read-JsonFile $FindingsPath
    $existingConv = Read-JsonFile $ConvergencePath
    $existingCycle = Read-JsonFile $CyclePath

    # Force-validation audit trail
    if ($ForceValidation) {
        $forceLogFile = Join-Path (Split-Path -Parent $FindingsPath) "force-validation-log.json"
        $forceLog = @()
        ...
        $forceLog += @{
            ...
            cycle = if ($existingCycle -and $existingCycle.current_cycle) { $existingCycle.current_cycle } else { "unknown" }
            reason = "Force-validation bypass activated"
            warning = "State integrity NOT guaranteed"
        }
    ...
    if ($ProposedFindingsData -and $ProposedFindingsData.findings) {
        $findingViolations = Validate-FindingStateIntegrity -ProposedFindings $ProposedFindingsData.findings -ExistingFindings $existingFindings
        foreach ($v in $findingViolations) {
            $blocked += $v
        }
    }

    if ($ProposedConvergenceData) {
        $gateViolations = Validate-GateEvidenceIntegrity -ProposedConvergence $ProposedConvergenceData -ExistingConvergence $existingConv
        foreach ($v in $gateViolations) {
            $blocked += $v
        }

        if ($ProposedConvergenceData.classification) {
            $oldClass = if ($existingConv -and $existingConv.classification) { [string]$existingConv.classification } else { "" }
            $newClass = [string]$ProposedConvergenceData.classification
            if (-not (Test-ValidClassificationTransition -FromClassification $oldClass -ToClassification $newClass)) {
                $blocked += "CLASSIFICATION TRANSITION VIOLATION: $oldClass -> $newClass is not an allowed classification transition."
            }
        }
    }

    $scopeWarnings = Test-AuditScopeIntegrity -ProjectPath $ProjectPath -ClaimedAuditedFileCount $AuditedFileCount
    foreach ($w in $scopeWarnings) { $warnings += $w }

    if ($blocked.Count -gt 0 -and -not $ForceValidation) {
        $errMsg = "STATE VALIDATION BLOCKED:`n" + ($blocked -join "`n")
        Write-Host $errMsg -ForegroundColor Red
        throw "State validation failed. $($blocked.Count) violation(s) detected. Use -ForceValidation to bypass (unsafe)."
    }
    ...
    if ($ProposedFindingsData) {
        Write-JsonFile $FindingsPath $ProposedFindingsData
        Write-Host "[STATE] findings.json written with validation" -ForegroundColor Green
    }
    if ($ProposedConvergenceData) {
        Write-JsonFile $ConvergencePath $ProposedConvergenceData
        Write-Host "[STATE] convergence.json written with validation" -ForegroundColor Green
    }
    if ($ProposedCycleData) {
        Write-JsonFile $CyclePath $ProposedCycleData
        Write-Host "[STATE] cycle.json written with validation" -ForegroundColor Green
    }

    if ($blocked.Count -gt 0) {
        Write-Host "[WARNING] $($blocked.Count) violation(s) were force-bypassed. State integrity NOT guaranteed." -ForegroundColor Yellow
    }
